...
* Understand algorithm decisions and how they affect runtime
...
## **Task 1: Testing and Implementation of ArrayList and MyAlgorithm [85 points]**
...
* `MyArrayListPublicTester.java`
    * Contains all the public test cases we will use to grade your MyArrayList and MyAlgorithm implementations (visible on Gradescope)
* `MyArrayListHiddenTester.java`
    * Contains only the headers and description to the hidden test cases we will use to grade your MyArrayList and MyAlgorithm implementations (hidden until the PA is graded) 
...
<tr>
   <td>
      <code>shiftToEnd</code>
   </td>
   <td>
      Test that the correct result is returned when the list alternates between elements that fill within the range and elements that don't.
   </td>
   <td>
      <ul>
         <li>
            ShiftToEnd when the input ArrayList is null
         </li>
         <li>
            ShiftToEnd when the offset is negative
         </li>
      </ul>
   </td>
</tr>
...
### Part 2: Implementation of ArrayList (60 points)
...
### **Part 3: MyAlgorithm (10 points)**

In this part of the assignment, you will use `MyArrayList` to solve a "challenge" programming problem. The goal of this section is to give you basic familiarity with the advantages/behavior of the data structure(s) you have just implemented.
...
#### Examples
<pre>
nums.data   -> [1, 2, 3, 4, 5, null]                        // Initial state
nums.size() -> 5
MyAlgorithm.shiftToEnd(nums, 3, 1)                          // Range is [2, 4]
nums.data   -> [1, 5, <b>2</b>, <b>3</b>, <b>4</b>, null]                        // 2, 3, and 4 moved to the end
nums.size() -> 5                                            // Size shouldn't change

nums.data   -> [1, 3, 4, 5, 2, null]                        // Initial state
nums.size() -> 5
MyAlgorithm.shiftToEnd(nums, 2, 1)                          // Range is [1, 3]
nums.data   -> [4, 5, <b>1</b>, <b>3</b>, <b>2</b>, null]                        // 1, 3, and 2 moved to the end
nums.size() -> 5                                            // Size shouldn't change
</pre>
...
Learning Goals:
- Intuitively understand why various methods take different amounts of time to run when given different input sizes.
- Understand when to use Java Arrays and when to use ArrayLists.

In this section, you will test your understanding of some of ArrayList's methods. For this purpose, we have provided the following file:
* `RuntimeTester.java`
    * Provides functionality to test the runtime of `add(0, element)`, which successively adds elements to the front of an ArrayList, and `append(element)`, which successively adds elements to the end of an ArrayList.
    * **Note:** Do not make any modifications to this file. You may view the source file, but you are not required to understand the code in this file.

You may assume that `add(0, element)` to add an element to the front of an ArrayList and `append(element)` to add an element to the end of an ArrayList are the same as covered in the lectures and zyBooks.
...
# Run
# <numIterations> - number of elements successively inserted into empty list (i.e. input size)
java RuntimeTester <numIterations>

# Example:
java RuntimeTester 100000
```
...
* Correctness (85 points) You will earn points based on the autograder tests that your code passes. If the autograder tests are not able to run (e.g., your code does not compile or it does not match the specifications in this writeup), you may not earn credit.
    * Tester (20 points)
    ...
    * MyAlgorithm (10 points)
        * The autograder will test your implementation of MyAlgorithm on both public and hidden test cases listed in the table
* Runtime (10 points)
